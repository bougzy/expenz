...
const userSchema = new mongoose.Schema({
    ...
    notifications: [{ message: String, date: { type: Date, default: Date.now } }], // New field for notifications
...
app.post('/api/transactions/approve/:id', authenticate, async (req, res) => {
    ...
    if (transaction && transaction.type === 'deposit') {
        ...
        // Update the user's profits and balance
        user.profits += transaction.amount; // Assuming profits increment equals the deposit amount
        user.balance += transaction.amount; // Update the user's balance as well
        await user.save();

        // Add notification
        user.notifications.push({
            message: `Your deposit of $${transaction.amount} has been approved. Your profits have increased!`,
        });
        await user.save();
    ...
    return res.status(400).json({ message: 'Transaction not found or not a deposit' });
});
...
app.post('/api/transactions/approve-withdraw/:id', authenticate, async (req, res) => {
    ...
    if (transaction && transaction.type === 'withdrawal') {
        ...
        // Update the user's profits and balance
        user.balance += transaction.amount; // Assuming withdrawal also impacts profits
        await user.save();

        // Add notification
        user.notifications.push({
            message: `Your withdrawal of $${Math.abs(transaction.amount)} has been approved. Your balance has been updated!`,
        });
        await user.save();
    ...
    return res.status(400).json({ message: 'Transaction not found or not a withdrawal' });
});
